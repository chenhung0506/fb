...
def setup_route(api):
    ...
    # api.add_resource(Default, '/')
    api.add_resource(BtcStaticResource, '/<path:filename>')
    api.add_resource(AdminStaticResource, '/admin/<path:filename>')
    api.add_resource(ValidUser, '/btc/valid')
    api.add_resource(LogOut, '/btc/logout')
...
class BtcStaticResource(Resource):
    def get(self, filename):
...
class Default(Resource):
    def get(self):
        # charactor = utils.createCaptcha()
        # log.info(charactor)
        # return render_template('index.html', value=charactor)
        # return render_template('index.html', value='test'), 200, {'Content-Type': 'text/css; charset=utf-8'}
